# ledger_app_polished_with_top3.py
# Polished dark UI + Top 3: Low stock alerts, Daily/Weekly summaries, Reconciliation tab - Jan 17, 2026
...
# Page config & CSS (same polished dark theme)
...
# Helpers (expanded for summaries)
def g_to_display(g): return g / OZ_TO_G if st.session_state.get("display_unit", "grams") == "ounces" else g
def display_to_g(v): return v * OZ_TO_G if st.session_state.get("display_unit", "grams") == "ounces" else v
...
def profit_margin(): return round(ledger["total_gross_profit"] / ledger["total_revenue"] * 100, 1) if ledger["total_revenue"] > 0 else 0.0
...
def add_transaction(desc, changes, cost_change=0.0, revenue=0.0, cogs=0.0, notes="", adjustment=False):
    ...
    entry = {
        "Date": now, "Description": desc + (" [ADJUSTMENT]" if adjustment else ""),
        "Notes": notes,
        **{k.capitalize() + " Δ (g)": v for k, v in changes.items() if k in ["vault", "will", "luke"]},
        ...
        "Revenue (\( )": revenue, "COGS ( \))": cogs, "Profit ($)": gross_profit
    }
...
if total_g < TOTAL_LOW:
    alerts.append(f"**Total stock low**: {g_to_display(total_g):.2f} ({'g' if st.session_state.display_unit == 'grams' else 'oz'}) — Replenish soon!")
if vault_g < VAULT_LOW:
    alerts.append(f"**Vault low**: {g_to_display(vault_g):.2f} — Secure backup running low!")
if min(vault_g, will_g, luke_g) < LOCATION_LOW:
    alerts.append("**One or more locations critically low** — Check immediately!")

if alerts:
    for msg in alerts:
        st.markdown(f"<p class='alert-red'>{msg}</p>", unsafe_allow_html=True)
    st.warning("Low stock alerts active — act fast!")
...
if not df_hist.empty:
    ...
    st.info("No transactions yet — summaries will appear after activity.")

st.divider()

# Tabs (Receive, Sell, Move, Cash/Team, + new Reconciliation)
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📥 Receive", "📤 Sell", "↔ Move", "💰 Adjustments", "🔍 Reconcile"])

with tab1:  # Receive (same as before)
...
# Sell, Move, Adjustments tabs (copy from previous complete version - abbreviated here for space; paste full if needed)

with tab5:  # New: Reconciliation
    st.subheader("Physical Count & Reconciliation")
    st.info("Enter actual counted weights below to compare with system totals.")
    recon_vault = st.number_input("Actual Vault (grams)", min_value=0.0, value=vault_g, step=0.1)
    recon_will = st.number_input("Actual Will (grams)", min_value=0.0, value=will_g, step=0.1)
    recon_luke = st.number_input("Actual Luke (grams)", min_value=0.0, value=luke_g, step=0.1)
    recon_reason = st.text_input("Reason for discrepancy (if any)", placeholder="e.g., spillage, miscount")

    if st.button("Reconcile & Adjust", use_container_width=True):
        ...
        if changes:
            add_transaction(f"Reconciliation adjustment: Vault {delta_v:+.2f}g, Will {delta_w:+.2f}g, Luke {delta_l:+.2f}g", 
                            changes, notes=recon_reason, adjustment=True)
            st.success("Reconciled & adjusted!")
        else:
            st.info("No discrepancies detected.")

# History & Export (same as before)
st.divider()
with st.expander("📜 History & Export"):
    if st.session_state.history:
        df = pd.DataFrame(st.session_state.history)
        csv = io.StringIO()
